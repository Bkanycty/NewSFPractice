...
- "images-layout": a directory representing a Python project that contains a "images"
  folder. Everything under the "images" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "images-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "images-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``images`` directory
        (or anything pointed by ``package_dir[""]``).

        The "images-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``images`` directory exists,
        this function will set ``package_dir[""] = "images"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "images"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`images-layout` detected -- analysing {src_dir}")
